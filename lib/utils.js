...
function fetchGitSha(basePath, cb) {
  var headPath = path.resolve(basePath, '.git', 'HEAD');

  // just let it return an err if the file does not exist
  fs.readFile(headPath, 'utf8', function(err1, headFileData) {
    if (err1) {
      return cb(err1);
    }
    if (headFileData.indexOf('ref: ') === 0) {
      var ref = headFileData.split(' ')[1].trim();
      var refPath = path.resolve(basePath, '.git', ref);
      fs.readFile(refPath, 'utf8', function(err2, refFileData) {
        if (err2) {
          return cb(err2);
        }
        cb(null, refFileData.trim());
      });
    } else {
      cb(new Error('HEAD could not be parsed for git repository'));
    }
  });
}
...
module.exports.fetchGitSha = fetchGitSha;