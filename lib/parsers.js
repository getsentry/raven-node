...
module.exports.parseRequest = function parseRequest(req, parseUser) {
  ...
  if (parseUser) {
    ...
    if (typeof parseUser === 'function') {
      ...
      var _user = req.user;
      // adds support to automatically convert mongoose objects
      if (_user.toObject) _user = _user.toObject();
      if (parseUser === true) {
        for (var key in _user) {
          if ({}.hasOwnProperty.call(_user, key)) {
            user[key] = _user[key];
          }
        }
      } else {
        parseUser.forEach(function (fieldName) {
          if ({}.hasOwnProperty.call(_user, fieldName)) {
            user[fieldName] = _user[fieldName];
          }
        });
      }
    }
