...
_.process = function process(kwargs, cb) {
    ...
    // this will happen asynchronously and we care about it's response.
    this._enabled && this.send(kwargs, function(err, result){
      if(err){
        return cb(err);
      } 
      var result = {'id': event_id, 'checksum': checksum};
      cb(null, result);
    });

};

_.send = function send(kwargs, cb) {
    var self = this;
    ...
    zlib.deflate(skwargs, function(err, buff) {
        ...
        self.transport.send(self, message, headers, cb);
    });
};
...
_.captureMessage = function captureMessage(message, kwargs, cb) {
    ...
    this.process(parsers.parseText(message, kwargs), function(err, result){
      if(err){
        return cb(err);
      }
      cb && cb(result);
...
_.captureException = function captureError(err, kwargs, cb) {
    ...
    parsers.parseError(err, kwargs, function(kw) {
        self.process(kw, function(err, result){
          if(err){
            return cb(err);
          }
          cb && cb(result);
