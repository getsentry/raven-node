...
var Client = function Client(dsn, options) {
    ...
    this.disableEnv = options.disableEnv || false;
    this.logEnvKeys = options.logEnvKeys || [];
...
_.process = function process(kwargs) {
    ...
    var http = kwargs['sentry.interfaces.Http'];

    if(http){
        if(this.disableEnv){
            delete http.env;
        }else if(this.logEnvKeys.length > 0){
            for(var key in http.env){
                if(this.logEnvKeys.indexOf(key) === -1){
                    delete http.env[key];
                }
            }
        }
