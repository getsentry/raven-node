...
Client.prototype.send = function (packet) {
    ...
    if (this._enabled) {
        ...
        var cache = [];
        var str = JSON.stringify(packet, function(key, value) {
        ...
        zlib.deflate(str, function(err, buff) {
            ...
            self.transport.send(self, auth, message, function(err) {
                if (err) {
                    // provide the packet for better outside diagnostics
                    err.packet = packet;
...
Client.prototype.captureError = function captureError(err, kwargs, cb) {
    ...
    interfaces.stacktrace(err, function(e, res) {
        ...
        // frames are sorted with most recent last
        if (frames && frames.length) {
            kwargs[interfaces.stacktrace.key] = res;
            var culrpit = frames[frames.length - 1];
            kwargs.culprit = [
                (culrpit.filename || 'unknown file').replace(process.cwd() + '/', ''),
                (culrpit.function || 'unknown function')
            ].join(':');
        }
